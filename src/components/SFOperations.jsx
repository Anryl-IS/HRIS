...
const SFOperations = () => {
  ...
  const handleUpdateIncident = async (e) => {
    e.preventDefault();
    const { error } = await supabase.from('ir_cases').update(selectedIncident).eq('id', selectedIncident.id);
    if (!error) {
      setIsEditing(false);
      fetchIRCases();
    }
        ...
        <div 
          ...
          <div 
            ...
            <div style={{ padding: '24px' }}>
              {isEditing ? (
                <form onSubmit={activeTab === 'incidents' ? handleEditIncident : handleUpdateCashLog} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  {activeTab === 'incidents' ? (
                   <>
                    ...
                    <>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          <select value={selectedIncident.payout_type} onChange={e => setSelectedIncident({...selectedIncident, payout_type: e.target.value})} style={{ padding: '10px', borderRadius: '8px', background: 'var(--bg-primary)', border: '1px solid var(--glass-border)', color: '#fff' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          <select value={selectedIncident.disbursement_method} onChange={e => setSelectedIncident({...selectedIncident, disbursement_method: e.target.value})} style={{ padding: '10px', borderRadius: '8px', background: 'var(--bg-primary)', border: '1px solid var(--glass-border)', color: '#fff' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
                          {selectedIncident.receipts?.map((url, i) => (
                            <div key={i} style={{ position: 'relative' }}>
                               <img src={url} alt="R" style={{ width: '60px', height: '60px', borderRadius: '4px', objectFit: 'cover' }} />
                               <button onClick={() => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px' }}>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                      ...
                      <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{activeTab === 'incidents' ? selectedIncident.area : `${selectedIncident.payout_type} (${selectedIncident.disbursement_method})`}</div>
                    </div>
                  </div>
                    ...
                    <div style={{ background: 'var(--bg-primary)', padding: '24px', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                      ...
                      {selectedIncident.memo_url && (
                        <div style={{ marginTop: '24px' }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', marginBottom: '12px' }}>Attachment Proof</div>
                          {selectedIncident.memo_url.toLowerCase().endsWith('.pdf') ? (
                            <a href={selectedIncident.memo_url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: 'rgba(255,255,255,0.05)', borderRadius: '8px', color: 'var(--accent-blue)' }}>
                                <FileText size={20} /> View PDFAttachment.pdf
                            </a>
                          ) : (
                            <img src={selectedIncident.memo_url} alt="Memo" style={{ width: '100%', borderRadius: '12px', border: '1px solid var(--glass-border)', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }} />
                          )}
                        </div>
                      )}
                    </div>
                        ...
                        {selectedIncident.receipts && selectedIncident.receipts.length > 0 && (
                            <div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px' }}>
                                    {selectedIncident.receipts.map((url, i) => (
                                        <div key={i} onClick={() => window.open(url, '_blank')} style={{ cursor: 'pointer', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--glass-border)', height: '120px' }}>
                                            <img src={url} alt="R" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        </div>
                                    ))}
                                </div>
                            </div>
              ...
              {activeTab === 'incidents' ? (
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Payout Type</label>
                        <select value={newCashLog.payout_type} onChange={e => setNewCashLog({ ...newCashLog, payout_type: e.target.value })} style={{ padding: '10px', borderRadius: '8px', background: 'var(--bg-primary)', border: '1px solid var(--glass-border)', color: '#fff' }}>
                           <option>Baranggay Share</option>
                           <option>Cash Assistance</option>
                           <option>Security Fund</option>
                           <option>Emergency Payout</option>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <select value={newCashLog.disbursement_method} onChange={e => setNewCashLog({ ...newCashLog, disbursement_method: e.target.value })} style={{ padding: '10px', borderRadius: '8px', background: 'var(--bg-primary)', border: '1px solid var(--glass-border)', color: '#fff' }}>
                           <option>G-Cash</option>
                           <option>Bank Transfer</option>
                           <option>Over Counter</option>
                           <option>Pera Padala</option>
