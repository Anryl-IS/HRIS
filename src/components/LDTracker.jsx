...
import { BookOpen, FileCheck, Calendar, Search, Plus, X, Award, Clock, ArrowRight, CheckCircle, Trash2, Edit3, User } from 'lucide-react';
...
const LDTracker = () => {
  ...
  const handleDeleteAssignment = async (id) => {
     if (window.confirm("Cancel this training assignment?")) {
        await supabase.from('ld_assignments').delete().eq('id', id);
        fetchLD();
     }
  };

  const filteredAssignments = assignments.filter(a => a.employee_name?.toLowerCase().includes(search.toLowerCase()) || a.department?.toLowerCase().includes(search.toLowerCase()));
  const filteredSubmissions = submissions.filter(s => s.employee_name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: 0 }}>
             <BookOpen size={28} color="var(--accent-purple)" /> Learning & Development Hub
          </h2>
          <div style={{ color: 'var(--text-secondary)', marginTop: '4px', fontSize: '0.9rem' }}>
             Tracking organizational standard SOP compliance & core training assignments.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', borderRadius: '12px', width: '300px' }}>
             ...
             <input type="text" placeholder="Search Employee or Dept..." value={search} onChange={e => setSearch(e.target.value)} style={{ background: 'transparent', border: 'none', color: '#fff', outline: 'none', width: '100%', fontSize: '0.9rem' }} />
          </div>
          <button onClick={() => setShowModal(true)} style={{ background: 'var(--accent-purple)', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '12px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', transition: 'all 0.2s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px' }}>
         <div className="glass-panel" style={{ padding: '24px', borderBottom: '4px solid var(--accent-purple)' }}>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 'bold', marginBottom: '8px' }}>COMPLIANCE SCORE</div>
            <div style={{ fontSize: '2rem', fontWeight: 'bold' }}>{completionRate}%</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--accent-purple)', marginTop: '4px' }}>Total Certifications issued</div>
         </div>
         <div className="glass-panel" style={{ padding: '24px', borderBottom: '4px solid var(--accent-red)' }}>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 'bold', marginBottom: '8px' }}>CRITICAL OVERDUE</div>
         ...
         <div className="glass-panel" style={{ padding: '24px', borderBottom: '4px solid var(--accent-blue)' }}>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 'bold', marginBottom: '8px' }}>ACTIVE LEARNERS</div>
            <div style={{ fontSize: '2rem', fontWeight: 'bold' }}>{assignments.length}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>Current active pipeline</div>
         </div>
         <div className="glass-panel" style={{ padding: '24px', borderBottom: '4px solid var(--accent-green)' }}>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 'bold', marginBottom: '8px' }}>VERIFIED SKILLS</div>
            <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--accent-green)' }}>{submissions.length}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>Successful certifications</div>
         </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.8fr', gap: '24px' }}>
          ...
          <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
             <div style={{ padding: '20px', borderBottom: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.02)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ margin: 0, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                   <Clock size={18} color="var(--accent-purple)" /> Pending Milestones
                </h3>
             </div>
             ...
             <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '550px', overflowY: 'auto' }}>
                {loading ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '24px', color: 'var(--text-secondary)' }}>No active assignments in this view.</div>
                  ...
                  <div key={a.id} className="glass-panel" style={{ padding: '16px', background: 'var(--bg-tertiary)', position: 'relative', border: '1px solid var(--glass-border)' }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                           <div style={{ fontWeight: 'bold', fontSize: '0.95rem' }}>{a.employee_name}</div>
                           <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '2px' }}>{a.department} &bull; {a.report_type}</div>
                        </div>
                     ...
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
                        ...
                        <span style={{ fontSize: '0.7rem', padding: '2px 8px', borderRadius: '4px', background: 'rgba(163, 113, 247, 0.1)', color: 'var(--accent-purple)', fontWeight: 'bold' }}>ASSIGNED</span>
                     </div>
                  </div>
                ))}
             </div>
          ...
          <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
             <div style={{ padding: '20px', borderBottom: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.02)' }}>
                <h3 style={{ margin: 0, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                   <Award size={18} color="var(--accent-green)" /> Verification Log (`ld_submissions`)
                </h3>
             </div>
               ...
               <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                 <thead>
                    <tr style={{ background: 'var(--bg-tertiary)', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                      <th style={{ padding: '16px' }}>Verified Professional</th>
                      <th style={{ padding: '16px' }}>Certification Type</th>
                      <th style={{ padding: '16px' }}>Completion Date</th>
                      <th style={{ padding: '16px' }}>Audit Status</th>
                    </tr>
                 </thead>
                 <tbody>
                    {loading ? (
                      <tr><td colSpan={4} style={{ padding: '32px', textAlign: 'center' }}>Syncing Audit Data...</td></tr>
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} className="glass-panel" style={{ width: '400px', padding: '32px' }}>
             <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
                <h3 style={{ margin: 0 }}>Issue Training Assignment</h3>
             ...
             <form onSubmit={handleAssign} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                   <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Employee Identity</label>
                   <input required value={newAssignment.employee_name} onChange={e => setNewAssignment({...newAssignment, employee_name: e.target.value})} style={{ background: 'var(--bg-primary)', border: '1px solid var(--glass-border)', color: '#fff', padding: '12px', borderRadius: '8px', outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                   <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>SOP Category</label>
                ...
                <button type="submit" style={{ background: 'var(--accent-purple)', color: '#fff', border: 'none', padding: '14px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '8px' }}>
                   <BookOpen size={18} /> Seal Assignment
                </button>
             </form>
          </div>
        </div>
